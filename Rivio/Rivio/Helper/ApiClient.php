...
class ApiClient 
{
  ...
  const RIVIO_OAUTH_TOKEN_URL   = "https://api.getrivio.com/oauth/token";
  const RIVIO_SECURED_API_URL   = "https://api.getrivio.com";
  const RIVIO_UNSECURED_API_URL = "http://api.getrivio.com";
  ...
  public function createApiPost($path, $data, $timeout=self::DEFAULT_TIMEOUT) {
    try 
    {
      ...
      $feed_url = self::RIVIO_SECURED_API_URL."/".$path;
      $http->setConfig($cfg);
  ...
  public function massCreatePurchases($orders)
  {
    $data = array();
    $data['platform'] = 'magento';
    $data['orders'] = $orders;
    //return $this->createApiPost("apps/".$this->_app_key."/purchases/mass_create", $data);
    return $this->createApiPost("api/postpurchase?api_key=".$this->_app_key."&secret_key=".$this->_secret, $data);
  }
}